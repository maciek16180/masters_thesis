...
class QANet:
    ...
    def __init__(self, voc_size, emb_init, dev_data=None, alphabet_size=128,
        ...
        if compile_train_fn:
            ...
            train_vars = inter_vars + [self.answer_starts_var,
                                       self.answer_ends_var, learning_rate_var]
            self.train_fn = theano.function(
                train_vars, train_loss, updates=updates)
    ...
    def get_start_probs(self, data, batch_size):
        ...
        for batch in self._iterate_minibatches(data, batch_size,
                                               with_answer_inds=False):
    ...
    (albo wsadzac jednobatchowe porcje danych), czyli batchowanie na zewnatrz)
    '''

    # Current implementation only allows for generating ends  for one start
    # index per example. To get end probs for k candidates for the start index,
    ...
    def train_one_epoch(self, train_data, batch_size, log_interval=10):
        ...
        for batch in self._iterate_minibatches(train_data, batch_size,
            ...
            if self.examples_since_last_checkpoint > self.checkpoint_examples:
                cp = len(self.dev_f1_log) + 1
                self.dev_f1_log.append(
                    self.calc_dev_f1(cp, verbose=False))
                if cp > 1 and self.dev_f1_log[-1] < self.dev_f1_log[-2]:
    ...
    def calc_dev_f1(self, checkpoint, batch_size=10, verbose=True, mode='cp'):
        ...
        def find_answer(par, ans_tokens):
            ...
            possible_starts = [
                m.start() for m in re.finditer(re.escape(ans_tokens[0]), par)]
        ...
        if self.predictions_path is not None:
            fname = 'pred.' + mode + '{:02d}.json'
            pred_path = os.path.join(self.predictions_path,
                                     fname.format(checkpoint))
    ...
    def _build_net(self, emb_char_filter_size=5, emb_dropout=True, **kwargs):
        ...
        if self.negative:
            l_c_emb = TrainNAWLayer(
                l_c_emb, l_c_mask, output_size=self.emb_size)
        ...
        l_c_char_conv = LL.Conv1DLayer(l_c_char_emb,
                                       ...
                                       pad=self.conv)
        # (batch_size * context_len x num_filters x
        #  context_word_len + filter_size - 1)

        l_c_char_emb = LL.ExpressionLayer(
            l_c_char_conv, lambda X: X.max(2), output_shape='auto')
        l_c_char_emb = LL.reshape(l_c_char_emb, (
            batch_size, context_len, self.num_emb_char_filters))
        ...
        l_q_char_conv = LL.Conv1DLayer(l_q_char_emb,
                                       ...
                                       b=l_c_char_conv.b,
                                       pad=self.conv)
        # (batch_size * question_len x num_filters x
        #  question_word_len + filter_size - 1)
        ...
        l_weighted_feat = WeightedFeatureLayer(
            [l_c_emb, l_q_emb, l_c_mask, l_q_mask])
        l_weighted_feat = LL.dimshuffle(l_weighted_feat, (0, 1, 'x'))

        # batch_size x context_len
        l_bin_feat = LL.InputLayer(
            shape=(None, None), input_var=self.bin_feat_var)
        ...
        # this is H from the paper, (batch_size * context_len x rec_size)
        ...
        # this is Z from the paper, (batch_size * question_len x rec_size)
        ...
        ''' Additional, weighted question encoding (alphas from the paper)'''
        ...
        l_alpha = MaskedSoftmaxLayer(LL.reshape(l_alpha, (
            batch_size, question_len)), l_q_mask)
    ...
    def _build_predictors(self, l_c_proj, l_z_hat):
        ...
        l_c_proj_resh = LL.reshape(l_c_proj, (
            self.batch_size, self.context_len, self.rec_size))
        ...
        l_start_soft = MaskedSoftmaxLayer(LL.reshape(l_Vs, (
            self.batch_size, self.context_len)), l_c_mask)
        ...
        l_end_feat = EndFeaturesLayer([l_c_proj_resh, l_z_hat,
                                       l_answer_starts])
        ...
        l_end_soft = MaskedSoftmaxLayer(LL.reshape(l_Ve, (
            self.batch_size, self.context_len)), l_c_mask)
    ...
    def _iterate_minibatches(self, inputs, batch_size, pad=-1, shuffle=False,
                             train=False, with_answer_inds=True):
