...
class QANet:
    ...
    def __init__(self, voc_size, alphabet_size, emb_size, emb_char_size, num_emb_char_filters, rec_size,
        ...
        test_start_probs = test_out[0][T.arange(context_var.shape[0]), answer_starts_var]
        test_end_conditional_probs = test_out[1][T.arange(context_var.shape[0]), answer_ends_var]
        ...
        if compile_train_fn:
            ...
            #print '    val_fn...'
            #
            #self.val_fn = theano.function([context_var, question_var, context_char_var, question_char_var, bin_feat_var,
            #                               mask_context_var, mask_question_var, mask_context_char_var,
            #                               mask_question_char_var, answer_starts_var, answer_ends_var],
            #                              test_loss)
    ...
    #def validate(self, val_data, batch_size, log_interval=10, premade_bin_feats=True):
    #    val_err = 0.
    #    val_batches = 0
    #    start_time = time.time()
    #
    #    for batch in iterate_minibatches(val_data, batch_size, self.pad_value, premade_bin_feats=premade_bin_feats):
    #        questions, contexts, questions_char, contexts_char, bin_feats, \
    #            question_mask, context_mask, question_char_mask, context_char_mask, answer_inds = batch
    #
    #        val_err += self.val_fn(contexts, questions, contexts_char, questions_char, bin_feats,
    #                               context_mask, question_mask, context_char_mask, question_char_mask,
    #                               answer_inds[:,0], answer_inds[:,1])
    #        val_batches += 1
    #
    #        if not val_batches % log_interval:
    #            print "Done {} batches in {:.2f}s".format(train_batches, time.time() - start_time)
    #
    ...
    def _calc_dev_f1(self, checkpoint, batch_size=10):
        ...
        res = os.system('python ' + self.eval_path + 'evaluate-v1.1.py ' + self.squad_path + 'dev-v1.1.json ' + path)
