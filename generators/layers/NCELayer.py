    ...
    def get_output_for(self, inputs, deterministic=False, **kwargs):
        ...
        if len(inputs) == 2:
            ...
            if not deterministic:
                ...
                true_part = T.max([true_logits, T.zeros_like(true_logits)],
                                  axis=0)
                true_part += -true_logits + T.log(
                    1 + T.exp(-T.abs_(true_logits)))
                sampled_part = T.max(
                    [sampled_logits, T.zeros_like(sampled_logits)], axis=0) + \
                    T.log(1 + T.exp(-T.abs_(sampled_logits)))
                sampled_part = sampled_part.sum(axis=1)
