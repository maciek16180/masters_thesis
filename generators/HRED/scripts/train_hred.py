...
idx_to_w, w_to_idx, voc_size, freqs = get_mt_voc(path=mt_path)
...
while len(last_scores) <= max_epochs_wo_improvement or last_scores[0] > min(last_scores) + tol:
    print '\n\nStarting epoch {}...\n'.format(epoch)
    train_error = net.train_one_epoch(train_data=train, batch_size=10, log_interval=200)
    val_error = net.validate(val_data=valid, batch_size=20)
...
test_error = net.validate(val_data=test, batch_size=20)
